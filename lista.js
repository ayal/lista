...
var lista = {list: [
    {name: 'filters', text:'Try these filters: ' + filter('basic') + filter('near') + filter('walking') + filter('jaffa') + filter('food') + filter('drink') + filter('street food')  + filter('club') + filter('24/7') + filter('transportation') + filter('SIM') + filter('all',''), tags:'*' },
...
{name: 'Barmitzvah', text:'Great place for drinking at night. prolly some food. prolly good food.', hours:'weekday nights and sat night maybe also :)', map:'https://www.google.com/maps?saddr=&daddr=' + encodeURIComponent('32.0594852,34.7701238') + '&dirflg=w', tags:'drink,snacks'},
